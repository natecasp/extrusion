...
function validate(wallMin, wallMax, idTol, wallTol){
  if((wall >= wallMin && wall <= wallMax) && (length <=60) && (lengthTol == 'min') && (Number(idTol) == Number(idTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    ...
    console.log('wallMin:' + wallMin);
    console.log(wallMin-wall);
    if ( Number(wall) < Number(wallMin) ) { console.log('triggered');$("#wall").css('border', '2px solid #ff6f6f'); } else {$("#wall").css('border', 'none');}
