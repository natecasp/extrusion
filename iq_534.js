...
function identifyCostSavings(wallMin, wallMax, refTol, wallTol, refTolerance, isSoft) {
  ...
  if(isSoft) { var identifier = 'OD'; var target= 'od-tol-2'} else {var identifier = 'ID'; var target = 'id-tol-2'}
...
function validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, isSoft){
  ...
  $("#4-week-price").trigger('change');
    ...
    greenlight.trigger('change'); 
    //Check Wall
    if ( wall < yellowWallMin ) { if( !$("#wall").hasClass('error') ){$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too Low').dialog("open");$("#wall").addClass("error");} else {console.log('already has error')}}
    else if ( wall > yellowWallMax ) { if( !$("#wall").hasClass('error') ){$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too High').dialog("open");$("#wall").addClass("error");} else {console.log('already has error')}}  else {$("#wall").removeClass("error");$("#wall-dialog").dialog("close");}
    //Check ID
