...
$(function() {
  ...
  $("#wall-dialog").dialog({
    autoOpen : false, modal : true, show : "blind", hide : "blind", title: "Out of spec",buttons: { 
                    OK: function() { 
                        $(this).dialog("close"); 
                    },
                    Continue: function() { 
...
function validate(wallMin, wallMax, idTol, wallTol){
  ...
  if((wall >= wallMin && wall <= wallMax) && (length <=60) && (length >=12) && ( id <= 0.300 ) && ( id >= 0.014 ) && (lengthTol == 'min') && (material !== '') && (Number(idTol) == Number(idTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    ...
    greenlight.trigger('change'); 
    //Check Wall
    if ( wall < wallMin ) { $("#wall").addClass("error"); $("#wall-dialog").dialog("open"); return false;} else {$("#wall").removeClass("error");}
    if ( wall > wallMax ) { $("#wall").addClass("error"); } else {if(wall > wallMin){$("#wall").removeClass("error");}}
    //Check ID
    ...
    //Check length
    if ( length > 60 ) { $("#length-2").addClass("error"); } else { $("#length-2").removeClass("error");}
    if ( length < 12 ) { $("#length-2").addClass("error"); } else { if(length < 60) {$("#length-2").removeClass("error");}}
    //Check Tolerances
