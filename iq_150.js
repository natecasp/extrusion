...
function validate(wallMin, wallMax, idTol, wallTol){
  ...
  if((wall >= wallMin && wall <= wallMax) && (length <=60) && (length >=12) && ( id <= 0.300 ) && ( id >= 0.014 ) && (lengthTol == 'min') && (material !== '') && (Number(idTol) == Number(idTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    ...
    if ( wall > wallMax ) { $("#wall").addClass("error"); if(!$('#wall-dialog').is(':visible')){$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too High').dialog("open");}} else {if(wall > wallMin){$("#wall").removeClass("error");}}
    //Check ID
    ...
    if ( length < 12 ) { $("#length-2").addClass("error"); if(!$('#length-dialog').is(':visible')){$("#length-dialog").dialog('option', 'title', 'Length Is Too High').dialog("open");}} else { if(length < 60) {$("#length-2").removeClass("error");}}
    //Check Tolerances
