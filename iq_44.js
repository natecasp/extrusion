...
function validate(wallMin, wallMax, idTol, wallTol){
  if((wall >= wallMin && wall <= wallMax) && (length <=60) && (lengthTol == 'min')) { 
    ...
    $("#length-tol-2").css('border', 'none');
    $("#id-tol-2").css('border', 'none');
    $("#wall-tol").css('border', 'none');
  } else { 
    greenlight.prop('checked', false); 
    greenlight.trigger('change'); 
    //Check Wall
    if ( wall < wallMin ) { $("#wall").css('border', '2px solid #ff6f6f'); }
    if ( wall > wallMax ) { $("#wall").css('border', '2px solid #ff6f6f'); }
    //Check length
    if ( length > 60 ) { $("#length-2").css('border', '2px solid #ff6f6f'); }
    //Check Tolerances
    if ( lengthTol !== 'min' ) { $("#length-tol-2").css('border', '2px solid #ff6f6f'); }
    if ( idTol !== idTolerance ) { $("#id-tol-2").css('border', '2px solid #ff6f6f'); }
    if ( wallTol !== wallTolerance ) { $("#wall-tol").css('border', '2px solid #ff6f6f'); }
  }
...
if (['Pebax 45D','Pebax 55D','Pebax 63D', 'Pebax 70D', 'Pebax 72D', 'Pebax 74D', 'Vestamid ML21 (Nylon 12)'].includes($("#material-2").val()) ){
  ...
  if(!isTol) {$("#id-tol-2").val(0.001); $("#wall-tol").val(0.0005);} var wallMin = 0.002; var wallMax = 0.010; var idTol = 0.001; var wallTol = 0.0005;
    validate(wallMin, wallMax, idTol, wallTol);
  }
  //CATEGORY 2 .051-.100
  if(id >= 0.051 && id <=0.100) { 
    if(!isTol) {$("#id-tol-2").val(0.001); $("#wall-tol").val(0.001);} var wallMin = 0.003; var wallMax = 0.015; var idTol = 0.001; var wallTol = 0.001;
    validate(wallMin, wallMax, idTol, wallTol);
  }
  //CATEGORY 3 .101-.150
  if(id >= 0.101 && id <=0.150) { 
    if(!isTol) {$("#id-tol-2").val(0.0015); $("#wall-tol").val(0.001);} var wallMin = 0.003; var wallMax = 0.020;  var idTol = 0.0015; var wallTol = 0.001;
    validate(wallMin, wallMax, idTol, wallTol);
  }
  //CATEGORY 4 .151-.200
  if(id >= 0.151 && id <=0.200) { 
    if(!isTol) {$("#id-tol-2").val(0.002); $("#wall-tol").val(0.0015);} var wallMin = 0.004; var wallMax = 0.025;  var idTol = 0.002; var wallTol = 0.0015;
    validate(wallMin, wallMax, idTol, wallTol);
  }
  //CATEGORY 5 .201-.250
  if(id >= 0.201 && id <=0.250) { 
    if(!isTol) {$("#id-tol-2").val(0.0025); $("#wall-tol").val(0.0015);} var wallMin = 0.005; var wallMax = 0.025;  var idTol = 0.0025; var wallTol = 0.0015;
    validate(wallMin, wallMax, idTol, wallTol);
  }
  //CATEGORY 6 .251-.300
  if(id >= 0.251 && id <=0.300) { 
    if(!isTol) {$("#id-tol-2").val(0.003); $("#wall-tol").val(0.002);} var wallMin = 0.006; var wallMax = 0.025;  var idTol = 0.003; var wallTol = 0.002;
    validate(wallMin, wallMax, idTol, wallTol);
  }
...
$("#id-tol-2, #wall-tol, #length-tol-2").on('change', function() {
  calculate(true);
});
