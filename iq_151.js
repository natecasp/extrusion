...
function validate(wallMin, wallMax, idTol, wallTol){
  ...
  if((wall >= wallMin && wall <= wallMax) && (length <=60) && (length >=12) && ( id <= 0.300 ) && ( id >= 0.014 ) && (lengthTol == 'min') && (material !== '') && (Number(idTol) == Number(idTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    ...
    greenlight.trigger('change'); 
    //Check Wall
    if ( wall < wallMin ) { $("#wall").addClass("error"); if(!$('#wall-dialog').is(':visible')){$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too Low').dialog("open");}} else {$("#wall").removeClass("error");}
    ...
    //Check length
    if ( length > 60 ) { $("#length-2").addClass("error"); if(!$('#length-dialog').is(':visible')){$("#length-dialog").dialog('option', 'title', 'Length Is Too High').dialog("open");}} else { $("#length-2").removeClass("error");}
