...
$("#id-2").on('input', function() {
  if($("#unit").val == 'in') {var max = 0.300; var min = 0.140} else {var max = 7.62; var min = 0.355;};
  if(this.value > max) this.value = null;
  if(this.value < min) this.value = null;
});
...
if($(":radio[value=wall]").is(":checked")) {
  $("#wall, #wall-range").val(((od-Number(getValue))/2).toFixed(wsig));
};
...
$("#od-3").on('input', function() {
  if($("#unit").val == 'in') {var max = 0.350; var min = 0.180} else {var max = 8.89; var min = 0.457;};
  if(this.value > max) this.value = null;
  if(this.value < min) this.value = null;
});
...
var getValue = $(this).val();
$("#wall-range").val(Number(getValue).toFixed(wsig));
var id = Number($("#id-range").val());
var od = Number($("#od-range").val());
...
if($(":radio[value=id]").is(":checked")) {
  ...
  $("wall, #wall-range").attr("max", (Number($("#od-3").val())-margin)/2);
  $("#id-2, #id-range").val((od-(Number(getValue)*2)).toFixed(sig));
  idController.setValue((od-(Number(getValue)*2)).toFixed(sig));
};

if($(":radio[value=od]").is(":checked")) {
  $("#od-3, #od-range").val((id+(Number(getValue)*2)).toFixed(sig));
