...
createShape(0.05);
updateJourney('Three JS objects: ' + scene.children.length);  
...
$("#id-ref-popup").click(function() {
  $("#id-ref-dialog").dialog("open");
});
$("#od-ref-popup").click(function() {
  $("#od-ref-dialog").dialog("open");
});
$("#wall-ref-popup-low").click(function() {
  $("#wall-ref-dialog-low").dialog("open");
});
$("#wall-ref-popup-high").click(function() {
  $("#wall-ref-dialog-high").dialog("open");
});
...
function checkMaterial(material) {
  if (material == 'Acetal') {
    $("#material-dialog").dialog('option', 'title', 'Acetal Warning').dialog("open");
  } 
  else {
...
function identifyCostSavings(wallMin, wallMax, refTol, wallTol, refTolerance, isSoft) {
  ...
  if(wall < wallMin || wall > wallMax) {
    ...
    $("#wall-savings p").text(message);
    $("#wall-savings").dialog('option', 'title', 'You could save $300!').dialog("open");
  ...
  if(wallTolerance < wallTol) {
    ...
    $("#wall-tol-savings p").text(message);
    $("#wall-tol-savings").dialog('option', 'title', 'You could save $300!').dialog("open");
  ...
  if(refTolerance < refTol) {
    ...
    $("#" + target).dialog('option', 'title', 'You could save $300!').dialog("open");
...
function checkRefDimension() {
  var ref = $('input[name=REF]:checked').val();
  if (['Pebax 45D','Pebax 55D','Pebax 63D', 'Pebax 70D', 'Pebax 72D', 'Pebax 74D', 'Vestamid ML21','Grilamid L25'].includes($("#material-2").val()) ){
  if(ref == 'id') {
    $("#id-3").siblings('span').addClass('error-ref');
    $("#id-ref-popup").removeClass('hide-price');
    $("#wall-ref-popup-high").addClass('hide-price');
    $("#wall-ref-dialog-high").dialog("close");
    $("#wall-2").siblings('span').removeClass('error-ref');
  } else if (ref == 'wall') {
    $("#wall-2").siblings('span').addClass('error-ref');
    $("#wall-ref-popup-high").removeClass('hide-price');
    $("#id-ref-popup").addClass('hide-price');
    $("#id-ref-dialog").dialog("close");
    $("#id-3").siblings('span').removeClass('error-ref');
  } else {
    $("#wall-2").siblings('span').removeClass('error-ref');
    $("#id-3").siblings('span').removeClass('error-ref');
    $("#wall-ref-popup-high").addClass('hide-price');
  ...
  if(ref == 'od') {
    $("#od-4").siblings('span').addClass('error-ref');
    $("#od-ref-popup").removeClass('hide-price');
    $("#wall-ref-popup-low").addClass('hide-price');
    $("#wall-ref-dialog-low").dialog("close");
    $("#wall-2").siblings('span').removeClass('error-ref');
  } else if (ref == 'wall') {
    $("#wall-2").siblings('span').addClass('error-ref');
    $("#wall-ref-popup-low").removeClass('hide-price');
    $("#od-ref-popup").addClass('hide-price');
    $("#od-ref-dialog").dialog("close");
    $("#od-4").siblings('span').removeClass('error-ref');
...
function validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, isSoft){
  ...
  var wallTolerance = $("#wall-tol").val();
  var length = Number($("#length-2").val());
  if((wall >= yellowWallMin && wall <= yellowWallMax) && (length <=60) && (length >=12) && (refTarget == true) && (qty <= 2500) && ( id <= 0.300 ) && ( id >= 0.014 ) && (lengthTol == 'min') && (material !== '') && (material !== 'Acetal') && (Number(refTolerance) >= Number(yellowRefTol)) && (Number(wallTolerance) >= Number(yellowWallTol))) { 
  ...
  var preBalloonPrice = Number($("#4-week-price").val());
    if($("#balloon-tubing").is(":checked")) {
      $("#4-week-price").val(preBalloonPrice + 300);
    }
  $("#4-week-price").trigger('change');
...
if (['Pebax 45D','Pebax 55D','Pebax 63D', 'Pebax 70D', 'Pebax 72D', 'Pebax 74D', 'Vestamid ML21','PET','Acetal','Grilamid L25'].includes($("#material-2").val()) ){
  var basePrice = 900;
    if (id < 0.014) {
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.001);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.001);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.0005);}} var yellowWallMin = 0.002; var yellowWallTol = 0.0005; var yellowWallMax = 0.01; var yellowRefTol = 0.001; var wallMin = 0.002; var wallMax = 0.010; var refTol = 0.001; var wallTol = 0.0005;
  ...
  if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.001);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.001);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.0005);}} var yellowWallMin = 0.0015; var yellowWallTol = 0.0005; var yellowWallMax = 0.0125; var yellowRefTol = 0.0005; var wallMin = 0.002; var wallMax = 0.010; var refTol = 0.001; var wallTol = 0.0005;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
  //CATEGORY 2 .051-.100
  if(id >= 0.051 && id <=0.100) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.001);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.001);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.001);}} var yellowWallMin = 0.0025; var yellowWallTol = 0.001; var yellowWallMax = 0.020; var yellowRefTol = 0.0005; var wallMin = 0.003; var wallMax = 0.015; var refTol = 0.001; var wallTol = 0.001;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
  //CATEGORY 3 .101-.150
  if(id >= 0.101 && id <=0.150) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.0015);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.0015);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.001);}} var yellowWallMin = 0.0025; var yellowWallTol = 0.0005; var yellowWallMax = 0.020; var yellowRefTol = 0.001; var wallMin = 0.003; var wallMax = 0.020;  var refTol = 0.0015; var wallTol = 0.001;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
  //CATEGORY 4 .151-.200
  if(id >= 0.151 && id <=0.200) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.002);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.002);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.0015);}} var yellowWallMin = 0.0025; var yellowWallTol = 0.0005; var yellowWallMax = 0.025; var yellowRefTol = 0.002; var wallMin = 0.004; var wallMax = 0.025;  var refTol = 0.002; var wallTol = 0.0015;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
  //CATEGORY 5 .201-.250
  if(id >= 0.201 && id <=0.250) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.0025);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.0025);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.0015);}} var yellowWallMin = 0.005; var yellowWallTol = 0.0015; var yellowWallMax = 0.025; var yellowRefTol = 0.0025; var wallMin = 0.005; var wallMax = 0.025;  var refTol = 0.0025; var wallTol = 0.0015;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
  //CATEGORY 6 .251-.300
  if(id >= 0.251 && id <=0.300) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.003);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.003);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.002);}} var yellowWallMin = 0.006; var yellowWallTol = 0.002; var yellowWallMax = 0.030; var yellowRefTol = 0.003; var wallMin = 0.006; var wallMax = 0.025;  var refTol = 0.003; var wallTol = 0.002;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
  if(id> 0.300) {
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.003);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.003);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.002);}} var yellowWallMin = 0.006; var yellowWallTol = 0.002; var yellowWallMax = 0.030; var yellowRefTol = 0.003; var wallMin = 0.006; var wallMax = 0.025;  var refTol = 0.003; var wallTol = 0.002;
...
//IF MATERIAL IS SOFT
 ...
 //CATEGORY 1 .014-.050
  ...
  if (id < 0.014) {
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.001);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.001);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.0005);}} var yellowWallMin = 0.002; var yellowWallTol = 0.0005; var yellowWallMax = 0.01; var yellowRefTol = 0.001; var basePrice = 1200; var wallMin = 0.003; var wallMax = 0.010; var refTol = 0.001; var wallTol = 0.0005;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
    
  }
  
  if(id >= 0.014 && id <=0.050) { 
   if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.001);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.001);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.0005);}} var yellowWallMin = 0.002; var yellowWallTol = 0.0005; var yellowWallMax = 0.0125; var yellowRefTol = 0.001; var basePrice = 1200; var wallMin = 0.003; var wallMax = 0.010; var refTol = 0.001; var wallTol = 0.0005;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
  ...
  //CATEGORY 2 .051-.100
  if(id >= 0.051 && id <=0.100) { 
     if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.001);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.001);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.001);}} var yellowWallMin = 0.003; var yellowWallTol = 0.0005; var yellowWallMax = 0.015; var yellowRefTol = 0.001; var basePrice = 1200;var wallMin = 0.003; var wallMax = 0.015; var refTol = 0.001; var wallTol = 0.001;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);

  }
  //CATEGORY 3 .101-.150
  if(id >= 0.101 && id <=0.150) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.0015);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.0015);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.001);}} var yellowWallMin = 0.003; var yellowWallTol = 0.001; var yellowWallMax = 0.020; var yellowRefTol = 0.0015; var basePrice = 1200; var wallMin = 0.003; var wallMax = 0.020;  var refTol = 0.0015; var wallTol = 0.001;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);

  }
  //CATEGORY 4 .151-.200
  if(id >= 0.151 && id <=0.200) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.002);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.002);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.0015);}} var yellowWallMin = 0.004; var yellowWallTol = 0.001; var yellowWallMax = 0.025; var yellowRefTol = 0.002; var basePrice = 1200; var wallMin = 0.004; var wallMax = 0.025;  var refTol = 0.002; var wallTol = 0.0015;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);

  }
  //CATEGORY 5 .201-.250
  if(id >= 0.201 && id <=0.250) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.0025);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.0025);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.0015);}} var yellowWallMin = 0.005; var yellowWallTol = 0.0015; var yellowWallMax = 0.025; var yellowRefTol = 0.0025; var basePrice = 1350; var wallMin = 0.005; var wallMax = 0.025;  var refTol = 0.0025; var wallTol = 0.0015;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
  
  }
  //CATEGORY 6 .251-.300
  if(id >= 0.251 && id <=0.300) { 
   if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.003);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.003);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.002);}} var yellowWallMin = 0.006; var yellowWallTol = 0.002; var yellowWallMax = 0.025; var yellowRefTol = 0.003; var basePrice = 1350; var wallMin = 0.006; var wallMax = 0.025;  var refTol = 0.003; var wallTol = 0.002;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
...
function validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, isSoft){
  ...
  var wallTolerance = $("#wall-tol").val();
  var length = Number($("#length-2").val());
  if((wall >= yellowWallMin && wall <= yellowWallMax) && (length <=1524) && (length >=304) && (refTarget == true) && (qty <= 2500) && ( id <= 7.62 ) && ( id >= 0.355 ) && (lengthTol == 'min') && (material !== '')  && (material !== 'Acetal') && (Number(refTolerance) >= Number(yellowRefTol)) && (Number(wallTolerance) >= Number(yellowWallTol))) { 
...
if (['Pebax 45D','Pebax 55D','Pebax 63D', 'Pebax 70D', 'Pebax 72D', 'Pebax 74D', 'Vestamid ML21','PET','Acetal','Grilamid L25'].includes($("#material-2").val()) ){
  var basePrice = 900;
    if (id < 0.355) {
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.025);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.025);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.013);}} var yellowWallMin = 0.050; var yellowWallTol = 0.0127; var yellowWallMax = 0.25; var yellowRefTol = 0.025; var wallMin = 0.050; var wallMax = 0.254; var refTol = 0.025; var wallTol = 0.0127;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
    
  }
  //CATEGORY 1 .355-.1.27
  if(id >= 0.355 && id <= 1.27) { 
  if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.025);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.025);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.013);}} var yellowWallMin = 0.0380; var yellowWallTol = 0.0127; var yellowWallMax = 0.3175; var yellowRefTol = 0.0127; var wallMin = 0.050; var wallMax = 0.254; var refTol = 0.025; var wallTol = 0.0127;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
  //CATEGORY 2 1.2954 - 2.54
  if(id >= 1.28 && id <= 2.54) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.025);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.025);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.025);}} var yellowWallMin = 0.0635; var yellowWallTol = 0.025; var yellowWallMax = 0.50; var yellowRefTol = 0.0127; var wallMin = 0.076; var wallMax = 0.381; var refTol = 0.025; var wallTol = 0.025;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
  //CATEGORY 3 2.5654 - 3.81
  if(id >= 2.55 && id <= 3.81 ) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.038);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.038);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.025);}} var yellowWallMin = 0.0635; var yellowWallTol = 0.0127; var yellowWallMax = 0.50; var yellowRefTol = 0.025; var wallMin = 0.076; var wallMax = 0.508;  var refTol = 0.038; var wallTol = 0.025;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
  //CATEGORY 4 3.8354 - 5.08
  if(id >= 3.82 && id <= 5.08) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.05);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.05);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.038);}} var yellowWallMin = 0.0635; var yellowWallTol = 0.0127; var yellowWallMax = 0.635; var yellowRefTol = 0.050; var wallMin = 0.102; var wallMax = 0.635;  var refTol = 0.050; var wallTol = 0.0380;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
  //CATEGORY 5 5.105 - 6.35
  if(id >= 5.09 && id <= 6.35) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.064);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.064);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.038);}} var yellowWallMin = 0.127; var yellowWallTol = 0.0380; var yellowWallMax = 0.635; var yellowRefTol = 0.064; var wallMin = 0.127; var wallMax = 0.635;  var refTol = 0.064; var wallTol = 0.0380;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
  //CATEGORY 6 6.375 - 7.62
  if(id >= 6.36 && id <= 7.62) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.076);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.076);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.05);}} var yellowWallMin = 0.1524; var yellowWallTol = 0.050; var yellowWallMax = 0.762; var yellowRefTol = 0.076; var wallMin = 0.152; var wallMax = 0.635;  var refTol = 0.076; var wallTol = 0.050;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, false);
  }
   if(id> 7.62) {
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.076);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.076);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.05);}} var yellowWallMin = 0.1524; var yellowWallTol = 0.050; var yellowWallMax = 0.762; var yellowRefTol = 0.076; var wallMin = 0.152; var wallMax = 0.635;  var refTol = 0.076; var wallTol = 0.050;
...
if (id < 0.355) {
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.025);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.025);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.013);}} var yellowWallMin = 0.050; var yellowWallTol = 0.0127; var yellowWallMax = 0.25; var yellowRefTol = 0.025; var wallMin = 0.076; var wallMax = 0.25; var refTol = 0.025; var wallTol = 0.0127;
    var basePrice = 1200;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
...
//CATEGORY 1 .355-.1.27
  if(id >= 0.355 && id <= 1.27) { 
  if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.025);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.025);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.013);}} var yellowWallMin = 0.050; var yellowWallTol = 0.0127; var yellowWallMax = 0.3175; var yellowRefTol = 0.025; var wallMin = 0.076; var wallMax = 0.25; var refTol = 0.025; var wallTol = 0.0127;
    var basePrice = 1200;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
  ...
  //CATEGORY 2 1.2954 - 2.54
  if(id >= 1.28 && id <= 2.54) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.025);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.025);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.025);}} var yellowWallMin = 0.076; var yellowWallTol = 0.0127; var yellowWallMax = 0.380; var yellowRefTol = 0.025; var wallMin = 0.076; var wallMax = 0.380; var refTol = 0.025; var wallTol = 0.025;
    var basePrice = 1200;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
  }
  //CATEGORY 3 2.5654 - 3.81
  if(id >= 2.55 && id <= 3.81 ) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.038);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.038);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.025);}} var yellowWallMin = 0.076; var yellowWallTol = 0.025; var yellowWallMax = 0.50; var yellowRefTol = 0.0380; var wallMin = 0.076; var wallMax = 0.50;  var refTol = 0.0380; var wallTol = 0.025;
    var basePrice = 1200;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
  }
  //CATEGORY 4 3.8354 - 5.08
  if(id >= 3.82 && id <= 5.08) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.050);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.050);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.038);}} var yellowWallMin = 0.102; var yellowWallTol = 0.025; var yellowWallMax = 0.635; var yellowRefTol = 0.050; var wallMin = 0.102; var wallMax = 0.635;  var refTol = 0.050; var wallTol = 0.0380;
    var basePrice = 1200;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
   ...
   //CATEGORY 5 5.105 - 6.35
  if(id >= 5.09 && id <= 6.35) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.064);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.064);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.038);}} var yellowWallMin = 0.127; var yellowWallTol = 0.0380; var yellowWallMax = 0.635; var yellowRefTol = 0.0635; var wallMin = 0.127; var wallMax = 0.635;  var refTol = 0.0635; var wallTol = 0.0380;
    var basePrice = 1350;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
  ...
  //CATEGORY 6 6.375 - 7.62
  if(id >= 6.36 && id <= 7.62) { 
    if(isTol == false) {if($("#id-tol-2").val() !== 'REF') { $("#id-tol-2").val(0.076);} if($("#od-tol-2").val() !== 'REF') { $("#od-tol-2").val(0.076);} if($("#wall-tol").val() !== 'REF') { $("#wall-tol").val(0.050);}} var yellowWallMin = 0.152; var yellowWallTol = 0.050; var yellowWallMax = 0.635; var yellowRefTol = 0.076; var wallMin = 0.152; var wallMax = 0.635;  var refTol = 0.076; var wallTol = 0.050;
    var basePrice = 1350;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
...
$("span.radio-button-label-3").removeClass('error-ref');
$(".image-167").addClass('hide-price');
$("span.blue").removeClass('hide-price');
$("p.white-text").removeClass('low-opacity');
$("#title-"+$(this).val()).addClass('hide-price');
$("#title-"+$(this).val()).parent().addClass('low-opacity');
$(".input-box, .tol-box").removeClass('low-opacity');
$(this).parent().siblings('div').addClass('low-opacity');
...
if($(this).val() == 'wall' && $(this).is(":checked")) {  $("#wall-tol").val('REF'); $("#wall, #wall-range, #wall-tol").prop('disabled', true); $("#id-2, #id-range, #od-3, #od-range, #od-tol-2, #id-tol-2").prop('disabled', false); if($("#id-tol-2").val() == 'REF') {$("#id-tol-2").val(0.001)}; if($("#od-tol-2").val() == 'REF') {$("#od-tol-2").val(0.001)}; calculate(false);}
...
if ($("#unit").val() == 'in'){var tol = 0.002;var sig = 3; var wsig = 4;}else{var tol= 0.05;var sig = 3; var wsig = 4;}
...
if($(":radio[value=wall]").is(":checked")) {
  $("#wall, #wall-range").val(((Number($("#od-3").val())-Number(this.value))/2).toFixed(wsig));
};
...
var od = Number($("#od-range").val());
if ($("#unit").val() == 'in'){var tol = 0.002;var sig = 3; var wsig = 4;var min = 0.014; var max = 0.300; var step = 0.001;}else{var tol= 0.05;var sig = 3; var wsig = 4;var min = 0.355; var max = 7.62; var step = 0.025;}
if(this.value > max) { this.value = max; }
else if(this.value < min) { this.value = min;}
$(this).val(Number((Math.round(this.value/step)*step).toFixed(sig)));
...
if($(":radio[value=wall]").is(":checked")) {
  $("#wall, #wall-range").val(((Number($("#od-3").val())-Number(getValue))/2).toFixed(wsig)).trigger('change');
};
...
if ($("#unit").val() == 'in'){var idMin = 0.014; var tol = 0.002;var sig = 3; var wsig = 4;}else{var idMin = 0.355; var tol= 0.05;var sig = 3; var wsig = 4;}
...
if($(":radio[value=wall]").is(":checked")) {
  $("#wall, #wall-range").val(((Number(this.value)-Number($("#id-2").val()))/2).toFixed(wsig));
};
...
if ($("#unit").val() == 'in'){var tol = 0.002;var sig = 3; var wsig = 4; var min = 0.018; var max = 0.350; var step = 0.001;}else{var tol= 0.05;var sig = 3; var wsig = 4;var min = 0.457; var max = 8.89; var step = 0.025;}
...
if($(":radio[value=wall]").is(":checked")) {
  $("#wall, #wall-range").val(((Number(getValue)-Number($("#id-2").val()))/2).toFixed(wsig)).trigger('change');
};
...
if ($("#unit").val() == 'in'){var tol = 0.002;var sig = 3; var wsig = 4; var max = 0.05; var min = 0.0005; var step = 0.0005;}else{var tol= 0.05;var sig = 3; var wsig = 4; var max = 1.27; var min = 0.0127; var step = 0.0125;}
if(this.value > max) { this.value = max;}
else if(this.value < min) { this.value = min;}
$(this).val(Number((Math.round(this.value/step)*step).toFixed(wsig)));
var getValue = $(this).val();
...
$("#balloon-tubing").on('change', function() {
  calculate(true);
});
...
if (['Pebax 45D','Pebax 55D','Pebax 63D','Pebax 72D','Vestamid ML21','PET','Acetal','Grilamid L25'].includes(material)) {
  if ($(".radio-button-label-3").hasClass("error-ref")) {
    return;
  } else {
...
$("#len-range, #length-2").attr({
"max": 100,
"step": 1
});
$("#wall-range, #wall").attr({
  "min": 0.0005,
  "max": 0.1000,
  "step": 0.0005
});
...
$("#id-range,#id-2").val(Number((Math.round((originalID / 25.4)/0.001)*0.001).toFixed(3)));
$("#len-range,#length-2").val(Number((originalLEN / 25.4).toFixed(0)));
$("#od-range,#od-3").val(Number((Math.round((originalOD / 25.4)/0.001)*0.001).toFixed(3)));
$("#wall, #wall-range").val(Number((Math.round((originalWALL / 25.4)/0.0005)*0.0005).toFixed(4)));
idController.setValue(Number((Math.round((originalID / 25.4)/0.001)*0.001).toFixed(3)));
odController.setValue(Number((Math.round((originalOD / 25.4)/0.001)*0.001).toFixed(3)));
console.log('od range: '+ $("#od-range").val() + ' id range: ' + $("#id-range").val() + ' wall range: ' +  $("#wall-range").val())
...
$("#id-range,#id-2").val(Number((Math.round((originalID * 25.4)/0.025)*0.025).toFixed(3)));
$("#len-range,#length-2").val(Number((originalLEN * 25.4).toFixed(0)));
$("#wall, #wall-range").val(Number((Math.round((originalWALL * 25.4)/0.0125)*0.0125).toFixed(4)));
$("#od-range, #od-3").val(Number((Math.round((originalOD * 25.4)/0.025)*0.025).toFixed(3)))
console.log('od range: '+ $("#od-range").val() + ' id range: ' + $("#id-range").val() + ' wall range: ' +  $("#wall-range").val())
odController.setValue(Number((Math.round((originalOD * 25.4)/0.025)*0.025).toFixed(3)));
idController.setValue(Number((Math.round((originalID * 25.4)/0.025)*0.025).toFixed(3)));
...
if($("#apply-expedite").is(":checked")) {
  ...
  if($("#balloon-tubing").is(":checked")) {
  var balloonTube = `style="display: block;margin-right:5px;" checked`;
  var balloonTubeLabel = `style="display:flex;"`;
  } else {
  var balloonTube = `style="display:none;margin-right:5px;"`;
  var balloonTubeLabel = `style="display:none;"`;
  }
...
<div class="row">
    ...
    <label class="label" `+ balloonTubeLabel +`><input type="checkbox" class="balloon-tube" id="l`+lineItem+`-balloon-tube" readonly `+ balloonTube +` onclick="return false"></input>Balloon Tubing</label>
	</div>
	<div class="col">
...
$("#total-price").val(total); 
  ...
  for (let i = 0; i < lines.length; ++i) {
    ...
    for (let a = 0; a < inputs.length; ++a) {
      ...
      var id = inputs[a].id;
      if (inputs[a].id.includes('greenlight') || inputs[a].id.includes('balloon-tube') || inputs[a].id.includes('expedite')) {
...
$('.line').filter(function() {
  return $(this).find('input[value="CC Processing Fee"]').length > 0;
}).remove();
