...
function validate(wallMin, wallMax, idTol, wallTol){
  ...
  if((wall >= wallMin && wall <= wallMax) && (length <=60) && (length >=12) && (odRef == true) && (qty <= 300) && ( id <= 0.300 ) && ( id >= 0.014 ) && ((lengthTol == 'min') || (Number(lengthTol) >= 0.25)) && (material !== '') && (Number(idTol) == Number(idTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    ...
    if ( length > 60 ) {console.log(length+100); if( !$("#length-2").hasClass('error') ){ $("#length-dialog").dialog('option', 'title', 'Length Is Too High').dialog("open");$("#length-2").addClass("error");} else {console.log('already has error')}} else { console.log('length reset');$("#length-2").removeClass("error");$("#length-dialog").dialog("close");}
    
    if ( length < 12 ) { console.log(length+100); if( !$("#length-2").hasClass('error') ){$("#length-dialog").dialog('option', 'title', 'Length Is Too Low').dialog("open");$("#length-2").addClass("error");} else {console.log('already has error')}}  else { if((length < 60) && (length > 12)) {console.log('length reset');$("#length-2").removeClass("error");$("#length-dialog").dialog("close");}}
    //Check Quantity
