...
function validate(wallMin, wallMax, idTol, wallTol){
  ...
  var idTolerance = $("#id-tol-2").val();
  var wallTolerance = $("#wall-tol").val();
  if((wall >= wallMin && wall <= wallMax) && (length <=1524) && (length >=304) && (odRef == true) && (qty <= 300) && ( id <= 7.62 ) && ( id >= 0.355 ) && ((lengthTol == 'min') || (Number(lengthTol) >= 0.6.35)) && (material !== '') && (Number(idTol) == Number(idTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    greenlight.prop('checked', true);
    greenlight.trigger('change'); 
    $("#wall,#length-2,#length-tol-2,#id-tol-2,#wall-tol,#id-2,#quantity-2").removeClass("error");
    $("#wall-dialog").dialog("close");
...
$("#material-2").on('change', function() {
  ...
  if (['HDPE','LDPE','Delrin','PET'].includes(this.value)) { $("#colorant option:first-child").prop("selectedIndex",0); $("input[id=color]").prop("checked", false).trigger("change"); $("#colorant").css('display', 'none'); } 
});
