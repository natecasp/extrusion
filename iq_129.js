...
function validate(wallMin, wallMax, idTol, wallTol){
  ...
  if((wall >= wallMin && wall <= wallMax) && (length <=60) && (length >=12) && ( id <= 0.300 ) && ( id >= 0.014 ) && (lengthTol == 'min') && (material !== '') && (Number(idTol) == Number(idTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    ...
    //Check ID
    if ( id > 0.300 ) { $("#id-2").css('border', '2px solid #ff6f6f'); } else {$("#id-2").css('border', 'none');}
    if ( id < 0.014 ) { $("#id-2").css('border', '2px solid #ff6f6f'); } else {if(id < 0.300){$("#id-2").css('border', 'none');}}
    //Check length
    if ( length > 60 ) { $("#length-2").css('border', '2px solid #ff6f6f'); } else { $("#length-2").css('border', 'none');}
    if ( length < 12 ) { $("#length-2").css('border', '2px solid #ff6f6f'); } else { if(length < 60) {$("#length-2").css('border', 'none');}}
    //Check Tolerances
...
<div class="row">
    ...
    <div class="item input-remove" id="price-line"><p class="label">Price($)</p><input class="quote-input price-item" id="l`+lineItem+`-price" value="`+price+`" readonly></input></div>
    ...
    <div class="item input-remove" id="lead-line"><p class="label">Lead Time</p><input class="quote-input" id="l`+lineItem+`-leadtime" value="`+leadtime.replace('-price','')+`" readonly></input>
  </div>
  </div>
</div>`;
...
$("#price-block").css('display', 'none');
  ...
  } else { createLine(); $("#quote-panel").css('display', 'flex');}
}
});
...
$("#custom-quote").on('change', function() {
  if ($(this).is(":checked")) {
    $("#price-block").css('display', 'none');
    $("#price-line, #lead-line").css('display','none');
  }
});
