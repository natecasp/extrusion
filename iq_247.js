...
function validate(wallMin, wallMax, idTol, wallTol){
  ...
  if((wall >= wallMin && wall <= wallMax) && (length <=60) && (length >=12) && (odRef == true) && (qty == 300) && ( id <= 0.300 ) && ( id >= 0.014 ) && (lengthTol == 'min') && (material !== '') && (Number(idTol) == Number(idTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    ...
    greenlight.trigger('change'); 
    //Check Wall
    if ( wall < wallMin ) { if(!$("#wall").classList.contains('error')) {$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too Low').dialog("open");} $("#wall").addClass("error");} else {$("#wall").removeClass("error");$("#wall-dialog").dialog("close"); }
    if ( wall > wallMax ) { if(!$("#wall").classList.contains('error')) {$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too High').dialog("open");} $("#wall").addClass("error");} else {if(wall > wallMin){$("#wall").removeClass("error");$("#wall-dialog").dialog("close");}}
    //Check ID
    ...
    //Check length
    if ( length > 60 ) { if(!$("#length-2").classList.contains('error')) {$("#length-dialog").dialog('option', 'title', 'Length Is Too High').dialog("open");} $("#length-2").addClass("error");} else { $("#length-2").removeClass("error");$("#length-dialog").dialog("close");}
    if ( length < 12 ) { if(!$("#length-2").classList.contains('error')) {$("#length-dialog").dialog('option', 'title', 'Length Is Too Low').dialog("open");} $("#length-2").addClass("error");} else { if(length < 60) {$("#length-2").removeClass("error");$("#length-dialog").dialog("close");}}
    //Check Quantity
    if ( qty !== 300 ) { if(!$("#quantity-2").classList.contains('error')) {$("#qty-dialog").dialog("open");} $("#quantity-2").addClass("error");} else { $("#quantity-2").removeClass("error"); $("qty-dialog").dialog("close");}
    //Check Tolerances
...
$(this).closest('span').addClass('hide-price');
