...
function validate(wallMin, wallMax, refTol, wallTol, isSoft){
  ...
  if(isSoft) {var refTolerance = $("#od-tol-2").val();} else {var refTolerance = $("#id-tol-2").val();};
  var wallTolerance = $("#wall-tol").val();
  var length = Number($("#length-2").val());
  if((wall >= wallMin && wall <= wallMax) && (length <=60) && (length >=12) && (odRef == true) && (qty <= 300) && ( id <= 0.300 ) && ( id >= 0.014 ) && (lengthTol == 'min') && (material !== '') && (Number(refTol) == Number(refTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
...
function validate(wallMin, wallMax, refTol, wallTol, isSoft){
  ...
  var wallTolerance = $("#wall-tol").val();
  var length = Number($("#length-2").val());
  if((wall >= wallMin && wall <= wallMax) && (length <=1524) && (length >=304) && (odRef == true) && (qty <= 300) && ( id <= 7.62 ) && ( id >= 0.355 ) && (lengthTol == 'min') && (material !== '') && (Number(refTol) == Number(refTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    $("#custom-quote").prop('checked', false);
    $("#custom-quote").trigger('change');
    greenlight.prop('checked', true);
    greenlight.trigger('change'); 
...
var wall = Number($("#wall-range").val());
console.log(Number(this.value)-(wall*2));
if((Number(this.value)-(wall*2)) < idMin) { $("#id-range,#id-2").val(idMin); e.preventDefault(); return false; }
