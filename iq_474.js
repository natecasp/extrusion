...
function checkRefTol(refTol, refTolerance, isSoft) {
  ...
  if ( Number(refTol) > Number(refTolerance) ) {console.log('refTol' + refTol + 'refTolerance'+refTolerance); $("#od-tol-2").addClass("error"); } else {$("#od-tol-2").removeClass("error"); console.log('remove error');}
  } else {
...
if($(this).is(":checked")) {
$(".white-text,.radio-button-label-3,.cert-title,.radio-button-label-2,.sales-rep,.text-block-38,.price-title,.cost,.text-block-37,.text-block-64,.text-block-65,.slider,.text-field-3,.leadtime,.shipping-title,.tol").addClass('light-text');
...
$(".white-text,.radio-button-label-3,.cert-title,.radio-button-label-2,.sales-rep,.text-block-38,.price-title,.cost,.text-block-37,.text-block-64,.text-block-65,.slider,.text-field-3,.leadtime,.shipping-title,.tol").removeClass('light-text');
...
calcShipping();
var getShipping = Number($("#shipping-total").val());
total += getShipping
$("#order-price").text(total);
$("#total-text").text(total);
if (user.model.expand.blanket_po){
 
$("#remaining").text(Number(user.model.expand.blanket_po[0].amount) - total);
if ( Number(user.model.expand.blanket_po[0].amount) - total <= -10000 ) {
...
$("#charge-to-blanket").click(function() {
  $("#terms").removeClass('hide-price');
  $("#blanket-po-number").val(user.model.blanket_po[0]);
  $("#new-blanket-po-amount").val(Number($("#remaining").text()));
  $("#blanket-po-name").val($("#blanket-number").text());
