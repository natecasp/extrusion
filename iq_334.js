...
function checkLenTol(lengthTol) {
  ...
  if ( Number(lengthTol) < limit ) { 
    $("#length-tol-2").addClass("error"); 
  } else if (lengthTol == 'min' || Number(lengthTol) > limit){ 
    $("#length-tol-2").removeClass("error");
}
}
function checkIdTol(idTol, idTolerance) {
...
if (['Pebax 45D','Pebax 55D','Pebax 63D', 'Pebax 70D', 'Pebax 72D', 'Pebax 74D', 'Vestamid ML21'].includes($("#material-2").val()) ){
  $("#4-week-price").val(900);
    if (id < 0.355) {
    if(isTol == false) {$("#id-tol-2").val(0.03); $("#wall-tol").val(0.013);} var wallMin = 0.0508; var wallMax = 0.254; var idTol = 0.03; var wallTol = 0.013;
    validate(wallMin, wallMax, idTol, wallTol);
  ...
  if(id >= 1.2954 && id <= 2.54) { 
    if(isTol == false) {$("#id-tol-2").val(0.03); $("#wall-tol").val(0.025);} var wallMin = 0.0762; var wallMax = 0.381; var idTol = 0.03; var wallTol = 0.025;
    validate(wallMin, wallMax, idTol, wallTol);
  }
  //CATEGORY 3 2.5654 - 3.81
  if(id >= 2.5654 && id <= 3.81 ) { 
    if(isTol == false) {$("#id-tol-2").val(0.04); $("#wall-tol").val(0.025);} var wallMin = 0.0762; var wallMax = 0.508;  var idTol = 0.04; var wallTol = 0.025;
    validate(wallMin, wallMax, idTol, wallTol);
  }
  //CATEGORY 4 3.8354 - 5.08
  if(id >= 3.8354 && id <= 5.08) { 
    if(isTol == false) {$("#id-tol-2").val(0.05); $("#wall-tol").val(0.038);} var wallMin = 0.1016; var wallMax = 0.635;  var idTol = 0.05; var wallTol = 0.038;
    validate(wallMin, wallMax, idTol, wallTol);
  }
  //CATEGORY 5 5.105 - 6.35
  if(id >= 5.105 && id <= 6.35) { 
    if(isTol == false) {$("#id-tol-2").val(0.06); $("#wall-tol").val(0.038);} var wallMin = 0.127; var wallMax = 0.635;  var idTol = 0.06; var wallTol = 0.038;
    validate(wallMin, wallMax, idTol, wallTol);
  }
  //CATEGORY 6 6.375 - 7.62
  if(id >= 6.375 && id <= 7.62) { 
    if(isTol == false) {$("#id-tol-2").val(0.08); $("#wall-tol").val(0.051);} var wallMin = 0.1524; var wallMax = 0.635;  var idTol = 0.08; var wallTol = 0.051;
    validate(wallMin, wallMax, idTol, wallTol);
  }
   if(id> 7.62) {
    if(isTol == false) {$("#id-tol-2").val(0.08); $("#wall-tol").val(0.051);} var wallMin = 0.1524; var wallMax = 0.635;  var idTol = 0.08; var wallTol = 0.051;
...
//CATEGORY 1 .355-.1.27
  if(id >= 0.355 && id <= 1.27) { 
  if(isTol == false) {$("#id-tol-2").val(0.0254); $("#wall-tol").val(0.0127);} var wallMin = 0.0508; var wallMax = 0.254; var idTol = 0.03; var wallTol = 0.013;
    validate(wallMin, wallMax, idTol, wallTol);
    $("#4-week-price").val(1200);
  ...
  //CATEGORY 2 1.2954 - 2.54
  if(id >= 1.2954 && id <= 2.54) { 
    if(isTol == false) {$("#id-tol-2").val(0.0254); $("#wall-tol").val(0.0254);} var wallMin = 0.0762; var wallMax = 0.381; var idTol = 0.03; var wallTol = 0.025;
    validate(wallMin, wallMax, idTol, wallTol);
    $("#4-week-price").val(1200);
  }
  //CATEGORY 3 2.5654 - 3.81
  if(id >= 2.5654 && id <= 3.81 ) { 
    if(isTol == false) {$("#id-tol-2").val(0.0381); $("#wall-tol").val(0.0254);} var wallMin = 0.0762; var wallMax = 0.508;  var idTol = 0.04; var wallTol = 0.025;
    validate(wallMin, wallMax, idTol, wallTol);
    $("#4-week-price").val(1200);
  }
  //CATEGORY 4 3.8354 - 5.08
  if(id >= 3.8354 && id <= 5.08) { 
    if(isTol == false) {$("#id-tol-2").val(0.0508); $("#wall-tol").val(0.0381);} var wallMin = 0.1016; var wallMax = 0.635;  var idTol = 0.05; var wallTol = 0.038;
    validate(wallMin, wallMax, idTol, wallTol);
    $("#4-week-price").val(1200);
  }
   //CATEGORY 5 5.105 - 6.35
  if(id >= 5.105 && id <= 6.35) { 
    if(isTol == false) {$("#id-tol-2").val(0.0635); $("#wall-tol").val(0.0381);} var wallMin = 0.127; var wallMax = 0.635;  var idTol = 0.06; var wallTol = 0.038;
    validate(wallMin, wallMax, idTol, wallTol);
    $("#4-week-price").val(1350);
  }
  //CATEGORY 6 6.375 - 7.62
  if(id >= 6.375 && id <= 7.62) { 
    if(isTol == false) {$("#id-tol-2").val(0.0762); $("#wall-tol").val(0.0508);} var wallMin = 0.1524; var wallMax = 0.635;  var idTol = 0.08; var wallTol = 0.051;
    validate(wallMin, wallMax, idTol, wallTol);
    $("#4-week-price").val(1350);
  }
  if(id> 7.62) {
