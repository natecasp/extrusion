...
function validate(wallMin, wallMax, idTol, wallTol){
  ...
  if((wall >= wallMin && wall <= wallMax) && (length <=60) && (length >=12) && ( id <= 0.300 ) && ( id >= 0.014 ) && (lengthTol == 'min') && (material !== '') && (Number(idTol) == Number(idTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    ...
    greenlight.trigger('change'); 
    //Check Wall
    if ( wall < wallMin ) { $("#wall").addClass("error"); if($('#wall-dialog').dialog('isOpen') === false){$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too Low').dialog("open");}} else {$("#wall").removeClass("error");$("#wall-dialog").dialog("close"); }
    if ( wall > wallMax ) { $("#wall").addClass("error"); if($('#wall-dialog').dialog('isOpen') === false){$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too High').dialog("open");}} else {if(wall > wallMin){$("#wall").removeClass("error");$("#wall-dialog").dialog("close");}}
    //Check ID
    ...
    //Check length
    if ( length > 60 ) { $("#length-2").addClass("error"); if(!$('#length-dialog').dialog('isOpen') === false){$("#length-dialog").dialog('option', 'title', 'Length Is Too High').dialog("open");}} else { $("#length-2").removeClass("error");$("#length-dialog").dialog("close");}
    if ( length < 12 ) { $("#length-2").addClass("error"); if(!$('#length-dialog').dialog('isOpen') === false){$("#length-dialog").dialog('option', 'title', 'Length Is Too High').dialog("open");}} else { if(length < 60) {$("#length-2").removeClass("error");$("#length-dialog").dialog("close");}}
    //Check Tolerances
