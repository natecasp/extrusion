...
function validate(wallMin, wallMax){
...
if($(":radio[value=id]").is(":checked")) {
  ...
  $("#id-2, #id-range").val(Number(od)-(Number(this.value)*2).toFixed(3));
  idController.setValue(Number(od)-(Number(this.value)*2).toFixed(3));
};

if($(":radio[value=od]").is(":checked")) {
  $("#od-3, #od-range").val(Number(id)+(Number(this.value)*2).toFixed(3));
  odController.setValue(Number(id)+(Number(this.value)*2).toFixed(3));
};
  calculate();
});
