...
<div class="row">
    ...
    <div class="item input-remove"><p class="label">Shipping Method</p><input class="quote-input shipping-line" id="l`+lineItem+`-shipping" value="`+shipping+`" readonly method="`+shipping+`" carrier="`+carrier+`" account="`+account+`"></input></div>
...
var shippingLines = $(".shipping-line");
let finalShipping = [];
let shippingArray = {};
priceMatchSmall = {
  "UPS Ground": 18,
...
for (let i = 0; i < shippingLines.length; ++i) {
  ...
  var price = 'price';
  var leadVal = $(shippingLines[i]).closest('.line-leadtime').val();
  var shipVal = $(shippingLines[i]).val();
  if ( $(shippingLines[i]).closest('.unit').val() == 'in' ) {
    if ( Number($(shippingLines[i]).closest('.line-length').val()) < 30 ) {
