...
if (['Pebax 45D','Pebax 55D','Pebax 63D', 'Pebax 70D', 'Pebax 72D', 'Pebax74D', 'Vestamid ML21 (Nylon 12)'].includes($("#material-2").val()) ){
  ...
  if(id >= 0.014 && id <=0.050) { 
    ...
    if((wall >= wallMin && wall <= wallMax) && (length <=60) && (length-tol == 'min')) { greenlight.prop('checked', true);greenlight.trigger('change'); } else { greenlight.prop('checked', false); greenlight.trigger('change'); }
  }
  //CATEGORY 2 .051-.100
