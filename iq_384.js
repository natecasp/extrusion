...
if((Number(params.OD) - Number(params.ID)) < 0) {
  var od = Number(params.ID) + tol;
  this.setValue(od);
} else {
  var od = Number(params.OD);
}
if((params.ID - params.OD) > 0) {
  var id = Number(params.OD) - tol;
  this.setValue(id);
} else {
...
if(this.value > max) { this.value = max; $("#wall, #wall-range").val(((od-Number(max))/2).toFixed(wsig)).trigger('change');}
else if(this.value < min) { this.value = min; $("#wall, #wall-range").val(((od-Number(min))/2).toFixed(wsig)).trigger('change');}
...
if($(":radio[value=od]").is(":checked")) {
  $("#od-3, #od-range").val((Number(getValue)+(wall*2)).toFixed(sig)).trigger('change');
...
if(this.value > max) { this.value = max;  $("#wall, #wall-range").val(((Number(max)-id)/2).toFixed(wsig)).trigger('change');}
else if(this.value < min) { this.value = min;  $("#wall, #wall-range").val(((Number(min)-id)/2).toFixed(wsig)).trigger('change');}
var getValue = $(this).val();
...
$("#id-range").val(expandedID.toFixed(sig));
$("#id-2").val(expandedID.toFixed(sig)).trigger('change');
}
...
odController.setValue(Number(getValue).toFixed(sig));
...
var getValue = $(this).val();
$("#wall-range").val(Number(getValue).toFixed(wsig)).trigger('input');
var id = Number($("#id-range").val());
var od = Number($("#od-range").val());
