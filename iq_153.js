...
$('#wall-dialog').dialog({
    autoOpen : false, show : "blind", hide : "blind", position: { 
    my: "left+10 top", 
    at: "right top",
    of: "#wall",
    collision: "none"
...
function validate(wallMin, wallMax, idTol, wallTol){
  ...
  var idTolerance = $("#id-tol-2").val();
  var wallTolerance = $("#wall-tol").val();
  if((wall >= wallMin && wall <= wallMax) && (length <=60) && (length >=12) && ( id <= 0.300 ) && ( id >= 0.014 ) && (lengthTol == 'min') && (material !== '') && (Number(idTol) == Number(idTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    greenlight.prop('checked', true);
    greenlight.trigger('change'); 
    $("#wall,#length-2,#length-tol-2,#id-tol-2,#wall-tol,#id-2").removeClass("error");
    ...
    greenlight.trigger('change'); 
    //Check Wall
    if ( wall < wallMin ) { $("#wall").addClass("error"); if($('#wall-dialog').dialog('isOpen') === false){$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too Low').dialog("open");}} else {$("#wall").removeClass("error");if($('#wall-dialog').dialog('isOpen')){$("#wall-dialog").dialog("close"); }}
    if ( wall > wallMax ) { $("#wall").addClass("error"); if($('#wall-dialog').dialog('isOpen') === false){$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too High').dialog("open");}} else {if(wall > wallMin){$("#wall").removeClass("error");if($('#wall-dialog').dialog('isOpen')){$("#wall-dialog").dialog("close"); }}}
    //Check ID
    ...
    //Check length
    if ( length > 60 ) { $("#length-2").addClass("error"); if($('#length-dialog').dialog('isOpen') === false){$("#length-dialog").dialog('option', 'title', 'Length Is Too High').dialog("open");}} else { $("#length-2").removeClass("error");if($('#length-dialog').dialog('isOpen')){$("#length-dialog").dialog("close"); }}
    if ( length < 12 ) { $("#length-2").addClass("error"); if($('#length-dialog').dialog('isOpen') === false){$("#length-dialog").dialog('option', 'title', 'Length Is Too High').dialog("open");}} else { if(length < 60) {$("#length-2").removeClass("error");if($('#length-dialog').dialog('isOpen')){$("#length-dialog").dialog("close"); }}}
    //Check Tolerances
...
function validate(wallMin, wallMax, idTol, wallTol){
  ...
  var idTolerance = $("#id-tol-2").val();
  var wallTolerance = $("#wall-tol").val();
  if((wall >= wallMin && wall <= wallMax) && (length <=1524) && (length >=304) && ( id <= 7.62 ) && ( id >= 0.355 ) && (lengthTol == 'min') && (material !== '') && (Number(idTol) == Number(idTolerance)) && (Number(wallTol) == Number(wallTolerance))) { 
    greenlight.prop('checked', true);
    greenlight.trigger('change'); 
    ...
    $("#id-2").css('border', 'none');
  } else { 
    greenlight.prop('checked', false); 
    greenlight.trigger('change'); 
    //Check Wall
    if ( wall < wallMin ) { $("#wall").css('border', '2px solid #ff6f6f'); } else {$("#wall").css('border', 'none');}
    if ( wall > wallMax ) { $("#wall").css('border', '2px solid #ff6f6f'); } else {if(wall > wallMin){$("#wall").css('border', 'none');}}
    //Check ID
    if ( id > 7.62 ) { $("#id-2").css('border', '2px solid #ff6f6f'); } else {$("#id-2").css('border', 'none');}
    if ( id < 0.355 ) { $("#id-2").css('border', '2px solid #ff6f6f'); } else {if(id < 7.62){$("#id-2").css('border', 'none');}}
    //Check length
    if ( length > 1524 ) { $("#length-2").css('border', '2px solid #ff6f6f'); } else { $("#length-2").css('border', 'none');}
    if ( length < 304 ) { $("#length-2").css('border', '2px solid #ff6f6f'); } else { if(length < 1524) {$("#length-2").css('border', 'none');}}
    //Check Tolerances
...
$('input[data-name=REF]').on('click', function() {
   ...
   var confirmation = confirm("Changing your 'REF' dimension?\n\nBy changing your 'REF' dimension, this quote will no longer qualify for instant pricing and online checkout and will instead become a custom quote.\n\nClick 'Ok' to proceed with a custom quote, or 'Cancel' to return.");
   if(!confirmation){
    return false;
   }   
  }
...
if($(this).val() == 'id' && $(this).is(":checked")) {  $("#id-2, #id-range, #id-tol-2").prop('disabled', true); $("#od-3, #od-range, #wall, #wall-range, #od-tol-2, #wall-tol").prop('disabled', false); $("#custom-quote").prop('checked', true);$("#custom-quote").trigger('change');}
...
if($(this).val() == 'wall' && $(this).is(":checked")) {  $("#wall, #wall-range, #wall-tol").prop('disabled', true); $("#id-2, #id-range, #od-3, #od-range, #od-tol-2, #id-tol-2").prop('disabled', false); $("#custom-quote").prop('checked', true);$("#custom-quote").trigger('change');}
});
...
$("#price-block").css('display', 'none');
  ...
  } else { createLine(); $("#price-block").css('display', 'none'); $("#quote-panel").css('display', 'flex');}
}
});
...
$("#custom-change").click(function() {
  ...
  if($('#editing').css('display') == 'none') { createLine(); } else { update(); }
...
$("#custom-quote").on('change', function() {
  if ($(this).is(":checked")) {
    $("#price-block").css('display', 'none');
    document.querySelectorAll('.input-remove').forEach(e => e.remove());
