...
function checkRefTol(yellowRefTol, refTolerance, isSoft) {
  ...
  if ( Number(yellowRefTol) > Number(refTolerance) ) {console.log('refTol' + yellowRefTol + 'refTolerance'+refTolerance); $("#od-tol-2").addClass("error"); } else {$("#od-tol-2").removeClass("error"); console.log('remove error');}
  } else {
  if ( Number(yellowRefTol) > Number(refTolerance) ) { $("#id-tol-2").addClass("error"); } else {$("#id-tol-2").removeClass("error");}
  }
}
function checkWallTol(wallTol, wallTolerance) {
   if ( Number(yellowWallTol) > Number(wallTolerance) ) { $("#wall-tol").addClass("error"); } else {$("#wall-tol").removeClass("error");}
}
...
if (unit == 'in') {
//VALIDATE
function validate(yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, isSoft){
  ...
  $("#4-week-price").trigger('change');
    ...
    greenlight.trigger('change'); 
    //Check Wall
    if ( wall < yellowWallMin ) { if( !$("#wall").hasClass('error') ){$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too Low').dialog("open");$("#wall").addClass("error");} }
    else if ( wall > yellowWallMax ) { if( !$("#wall").hasClass('error') ){$("#wall-dialog").dialog('option', 'title', 'Wall Thickness Too High').dialog("open");$("#wall").addClass("error");} }  else {$("#wall").removeClass("error");$("#wall-dialog").dialog("close");}
...
//IF MATERIAL IS SOFT
 ...
 //CATEGORY 1 .014-.050
  ...
  if(id >= 0.014 && id <=0.050) { 
   if(isTol == false) {$("#od-tol-2").val(0.001); $("#wall-tol").val(0.0005);} var yellowWallMin = 0.0015; var yellowWallTol = 0.0005; var yellowWallMax = 0.054; var yellowRefTol = 0.001; var basePrice = 1200; var wallMin = 0.002; var wallMax = 0.010; var refTol = 0.001; var wallTol = 0.0005;
    validate(basePrice, yellowWallMin, yellowWallMax, yellowRefTol, yellowWallTol, wallMin, wallMax, refTol, wallTol, true);
...
function validate(wallMin, wallMax, refTol, wallTol, isSoft){
  ...
  var wallTolerance = $("#wall-tol").val();
  var length = Number($("#length-2").val());
  if((wall >= wallMin && wall <= wallMax) && (length <=1524) && (length >=304) && (refTarget == true) && (qty <= 2500) && ( id <= 7.62 ) && ( id >= 0.355 ) && (lengthTol == 'min') && (material !== '') && (Number(refTolerance) >= Number(refTol)) && (Number(wallTolerance) >= Number(wallTol))) { 
    $("#custom-quote").prop('checked', false);
    $("#custom-quote").trigger('change');
    greenlight.prop('checked', true);
    greenlight.trigger('change'); 
...
if (['Pebax 45D','Pebax 55D','Pebax 63D', 'Pebax 70D', 'Pebax 72D', 'Pebax 74D', 'Vestamid ML21','PET','Delrin','Grilamid L25'].includes($("#material-2").val()) ){
  if(Number($("#quantity-2").val()) == 300) {
    ...
    $("#4-week-price").val(1950);
  } else if (Number($("#quantity-2").val()) == 2500) {
    $("#4-week-price").val(3100);
  }
    if (id < 0.355) {
  ...
  if(isTol == false) {$("#od-tol-2").val(0.03); $("#wall-tol").val(0.013);} var wallMin = 0.0508; var wallMax = 0.254; var refTol = 0.03; var wallTol = 0.013;
    validate(wallMin, wallMax, refTol, wallTol, true);
     if(Number($("#quantity-2").val()) == 300) {
        ...
        $("#4-week-price").val(2250);
      } else if (Number($("#quantity-2").val()) == 2500) {
        $("#4-week-price").val(3400);
      }
  ...
  if(id >= 5.105 && id <= 6.35) { 
    ...
    validate(wallMin, wallMax, refTol, wallTol, true);
     if(Number($("#quantity-2").val()) == 300) {
        ...
        $("#4-week-price").val(2400);
      } else if (Number($("#quantity-2").val()) == 2500) {
        $("#4-week-price").val(3550);
      }
  }
  //CATEGORY 6 6.375 - 7.62
  if(id >= 6.375 && id <= 7.62) { 
